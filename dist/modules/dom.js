...
function createUserCard(user) {
    ...
    const userImage = createHTMLElement({ tag: 'img', className: 'card__img' });
...
function createWeatherInfoHolderAndPopulate({ temperature, humidity, condition }) {
    const weatherInfoHolder = createHTMLElement({ tag: 'div', className: 'weather-info' });
    const tempP = createHTMLElement({ tag: 'p',
        className: 'temp',
    ...
    const conditionP = createHTMLElement({ tag: 'p',
        className: 'condition',
...
function toggleLoaderAndContent(isLoading) {
    ...
    if (!(loader instanceof HTMLElement) || !(mainContent instanceof HTMLElement)) {
...
async function updateWeatherInfo() {
    ...
    for (let i = 0; i < cardsArr.length; i++) {
        ...
        const weatherConditions = await getWeather({ latitude: cachedUsers[i].weather?.latitude,
            longitude: cachedUsers[i].weather?.longitude });
        if (!weatherConditions) {
            const errorMessageParagraph = createHTMLElement({ tag: 'p',
                className: 'error-weather',
                content: 'Weather conditions unavailable at this time.' });
        ...
        if (weatherInfoHolder) {
            ...
            if (!errorMessageParagraph) {
                ...
                const humidityP = createHTMLElement({ tag: 'p',
                    className: 'humidity',
...
export async function setUpRefreshWeatherTimer() {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            error instanceof Error ? console.error('Weather update failed:' + error.message) : console.error(String(error));
        }
        finally {
            isUpdating = false;
        }
